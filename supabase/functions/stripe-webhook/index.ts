...
const upsertPriceRecord = async (price: Stripe.Price) => {
  ...
  if (data?.length) priceData.id = data.id;
...
const checkoutSessionCompletedEvent = async (event: Stripe.Event) => {
  ...
  if (session.node !== 'subscription') return;
...
Deno.serve(async request => {
  ...
  try {
    ...
    if (!webhookSecret) return new Response('Webhook Error: Missing Stripe webhook sercret', { status: 400 });
  ...
  if (relevantEvents.has(receivedEvent.type)) {
    try {
      switch (receivedEvent.type) {
        ...
        case 'price.created':
        case 'price.updated':
          await upsertPriceRecord(receivedEvent?.data?.object as Stripe.Product);
          break;
